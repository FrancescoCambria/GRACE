...
def pattern_to_text(pattern, names, anchor_label):
    ...
    parts = [p.strip() for p in pattern.split(';')] # Changed to semicolon as law rules seem to use it
...
def translate_rule(row):
    ...
    return f"{subject} {body_text} than are also {head_text} with support {support} and confidence {confidence}"
